...
using namespace std;
//TrybTestowania - klasa do uzycia menu do trybu testowania programu
//Implementacja menu dla pracy z generowanymi danymi
void TrybTestowania::uruchomTestowanie(int*& tabela, float*& tabela_float, int& liczba_elementow, int& liczba_elementow_float) {
	int liczba_sortowan;
	cout << "Podaj liczbe testowan: " << endl;
	cin >> liczba_sortowan;
poczatek:
	int wybor;
	while (true) {
		...
		//Uzycie sortowania dla odpowiednich przypadkow dla tabeli typu integer dla kilkakrotnego sortowania
		if (wybor == 1 || wybor == 2 || wybor == 3 || wybor == 4 || wybor == 5 || wybor == 6 ||  wybor == 15) {
			bool kontynuj = OpcjeTestowania::sortujTabeleTestowanie(tabela, liczba_elementow, liczba_sortowan, wybor);
			if (!kontynuj) {
				break;
			}
		}
		//Uzycie sortowania dla odpowiednich przypadkow dla tabeli typu float dla kilkakrotnego sortowania
		if (wybor == 8 || wybor == 9 || wybor == 10 || wybor == 11 || wybor == 12 || wybor == 13 || wybor == 16) {
			bool kontynuj = OpcjeTestowania::sortujTabeleFloatTestowanie(tabela_float, liczba_elementow_float, liczba_sortowan, wybor);
			if (!kontynuj) {
				break;
			}
		}
